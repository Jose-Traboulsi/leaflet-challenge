...
d3.json("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_week.geojson").then(function (data) {
  ...
  function styleInfo(feature) {
    ...
    return {
      opacity: 1,
      fillOpacity: 0.6,
      fillColor: getColor(depth), // Color based on earthquake depth
      color: "#000000", // Black outline
      radius: getRadius(magnitude), // Radius/size based on eathquake magnitude
      stroke: true,
      weight: 0.5
    };
  }
  ...
  function getColor(depth) {
    ...
    else if (depth <= 30) return "#ADFF2F"; // YellowGreen
    else if (depth <= 50) return "#FFFF00"; // Yellow
    else if (depth <= 70) return "#FFA500"; // Orange
    else if (depth <= 90) return "#FF4500"; // RedOrange
